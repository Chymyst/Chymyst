...
class ChymystPresentation extends FlatSpec with Matchers {
  ...
  - make (1, 2) and (3, 4) available as concurrent data
  - somehow say that these are actually inputs for two parallel runs of `plus`
  - somehow say that `times` should get its input from the results of `plus`

  Imagine we could write (in a modified Scala):

  def_concur plus(inp: (Int, Int)) = ... // Defines `plus()` as concurrent function.

  def emit(x: _) = ...  // Makes `x` somehow available as "concurrent data".
  ...
  We do not directly write `plus((1, 2))`: We already declared `plus` as concurrent,
  so now `plus` should somehow consume its input data automatically.
  ...
  Instead of `emit((1, 2))`, we will have to say `emit("inp", (1, 2))`.
  ...
  def_concur times(inp2: (Int, Int)): Int = ... // Use `inp2` as input name.
  ...
  def_concur plus(inp: (Label, Int, Int)): Unit = {
    val (output_label, x, y) = inp
    emit(output_label, x + y)
  }

  def_concur times(inp1: Int, inp2: Int): Unit = println(inp1 * inp2)
  ...
  What we have achieved with this DSL design:
  ...
  - the runtime system will take care of scheduling and create threads as needed
  ...
  - To define labels, we could use the constructors of the class `M`:
  */
  ...
  - This is implemented via `M#apply()`.
  ...
  */
  ...
  Type checking of pattern variables is enforced in the standard Scala manner.

  - Now define our `plus` function and its input data label, `inp`:
  */
  val inp = new M[(M[Int], Int, Int)]("input for plus")

  val plus = go { case inp((outputLabel, x, y)) ⇒ outputLabel(x + y) }
  /*
  Now, the concurrent functions `plus` and `times` are declaratively defined as
  values. We need to tell the runtime system somehow to "activate" them, so that
  the runtime will begin to wait for their input data.

  It is convenient to make the "activation" into a separate step in the DSL.

  The activation method is called `site()` that takes a list of declared functions:
  */
  site(plus, times)
  /*
  The motivation for the name `site` is that we are defining a "holding site" for
  concurrent data, where the emitted data items with labels `inp`, `inp1`, `inp2`
  will gather and wait to be consumed by concurrent functions we declared.
  ...
  */
  ...
  /*
  Example 2: A computation that processes arriving data items concurrently.

  Suppose we expect data items carrying `Double` values x_1, x_2, ...
  ...
  We model x_1, x_2, ... as "concurrent data" inputs. They need a label: say, `inp`.
  They can share the same label, because the computation `cos(x_i)` is the same
  and needs to be run in parallel.
  */
  it should "run example 2" in {
    ...
    */
    ...
    We need to update `max_cos` concurrently, but how?
    ...
    */
    ...
    How will we signal that we want to read that value? The only way to signal
    ...
    The `Promise` value can be passed as concurrent data with the `read` label:
     */
    ...
    // Wait for computations to finish. Brittle! But this is just a first test.
    Thread.sleep(500)
